...
module.exports = {
    async results(_, args, ctx) {
        return db
            ...
            .innerJoin('training', 'training.team_id', 'teams.id')
    ...
    async result(_, { filter }, ctx) {
        ...
        if (id) {
            ...
            return db
                ...
                .innerJoin('team_category', 'team_category.id', 'teams.category_id')
                .innerJoin('training', 'training.team_id', 'teams.id')
                .whereRaw(`training.dt_training = '${dt_result}' AND team_category.user_id = ${ctx.user.id}`)
        } else {
            return null
        }
    }
