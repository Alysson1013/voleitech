    ...
    async training_type(_, { filter }, ctx) {
        ...
        if (id) {
            return db
                ...
                .innerJoin('teams', 'teams.id', 'training.team_id')
                .leftJoin('team_category', 'team_category.id', 'teams.category_id')
                .whereRaw(`training_type.id = ${id} AND training_type.id AND team_category.user_id = ${ctx.user.id}`)
                .first()
            ...
            return db
                ...
                .innerJoin('teams', 'teams.id', 'training.team_id')
                .leftJoin('team_category', 'team_category.id', 'teams.category_id')
                .whereRaw(`training_type.training_type_name = '${training_type_name}' AND training_type.id AND team_category.user_id = ${ctx.user.id}`)
                .first()
        } else {
            return null
        }
